...
import com.android.QKSMS.interactor.*
...
import com.android.QKSMS.model.SyncLog
...
//    syncRepository: SyncRepository,
    private val markAllUnLocked: MarkAllUnLocked,
    ...
    private val markLocked: MarkLocked,
...
//        ratingManager.addSession()
    ...
    override fun bindView(view: MainView) {
        ...
        when {
            ...
            !permissionManager.hasReadSms() || !permissionManager.hasContacts() || !permissionManager.hasCalendar() -> view.requestPermissions()
        }
...
//                .subscribe { navigator.showChangelog() }
        ...
        view.activityResumedIntent
                ...
                .switchMap {
                    ...
                    prefs.keyChanges
                            .filter { key -> key.contains("avatar") }
                            .doOnNext { view.themeChanged() }
...
//                }
        ...
        view.optionsItemIntent
                ...
                .withLatestFrom(view.conversationsSelectedIntent) { _, conversation ->
                    if (view.isSecure()) {
                        markLocked.execute(conversation)
                        view.clearSelection()
                    } else {
                        view.toSecuritySetting()
                    }
                }
                .autoDisposable(view.scope())
                .subscribe()
        ...
        view.optionsItemIntent
                ...
                .withLatestFrom(view.conversationsSelectedIntent) { _, conversation ->
                    if (view.isSecure()) {
                        view.checkSecurity(conversation)
                        view.clearSelection()
                    }
                }
                .autoDisposable(view.scope())
...
//                .subscribe { ratingManager.dismiss() }
        ...
        view.conversationsSelectedIntent
                .withLatestFrom(state) { selection, state ->
                    ...
                    val locked = conversations.sumBy { if (it.locked) 1 else -1 } > 0

                    val data = conversationRepo.getConversations(archived = false, locked = locked)

                    when (state.page) {
                        is Inbox -> {
                            val page = state.page.copy(addContact = add, markPinned = pin, markRead = read, selected = selected, locked = locked, data = data)
                            newState { copy(page = page) }
                        }

                        is Archived -> {
                            val page = state.page.copy(addContact = add, markPinned = pin, markRead = read, selected = selected, locked = locked)
                            newState { copy(page = page) }
                        }
                    }
                }
