...
import com.android.QKSMS.common.util.ClipboardUtils
import com.android.QKSMS.common.util.MessageDetailsFormatter
import com.android.QKSMS.common.util.Utils
import com.android.QKSMS.common.util.extensions.makeToast
import com.android.QKSMS.compat.SubscriptionManagerCompat
import com.android.QKSMS.compat.TelephonyCompat
import com.android.QKSMS.extensions.*
...
import com.android.QKSMS.util.ActiveSubscriptionObservable
    ...
    init {
        ...
        disposables += attachments
                .subscribe { attachments ->
                    run {
                        newState { copy(attachments = attachments) }
    ...
    override fun bindView(view: ComposeView) {
        ...
        view.optionsItemIntent
                ...
                .withLatestFrom(selectedChips) { _, chips ->
                    view.showContactsPlus(sharing, chips)
                }
                .autoDisposable(view.scope())
                .subscribe()
        ...
        // Audio attachment clicks
        view.messagePartClickIntent
                .mapNotNull(messageRepo::getPart)
                .filter { part -> part.isAudio() }
                .autoDisposable(view.scope())
                .subscribe { part ->
                    run {
                        player = if(player != null) {
                            ...
                            Utils.playMp3(Utils.LoadRaw(context, part.getUri()))
                        }
                    }
                }
        ...
        view.scheduleSelectedIntent
                ...
                .subscribe { scheduled ->
                    run {
                        newState { copy(scheduled = scheduled) }
                        newState { copy(attaching = false) }
                    }
                }
        ...
        // Attach a record
        view.audioContentIntent
                .map { uri -> Attachment.Audio(uri) }
                .withLatestFrom(attachments) { attachment, attachments -> attachments + attachment }
                .subscribeOn(Schedulers.io())
                .autoDisposable(view.scope())
                .subscribe(attachments::onNext) { error ->
        ...
        view.textChangedIntent
                ...
                .mapNotNull { draft -> tryOrNull { SmsMessage.calculateLength(draft, false) } }// prefs.unicode.get()
                .map { array ->
        ...
        view.emoji
                .autoDisposable(view.scope())
                .subscribe{ view.emojiPopup.toggle() }
        ...
        view.optionsItemIntent
                ...
                .withLatestFrom(state) { _, state ->
                    when {
                        ...
                        state.attaching -> newState { copy(attaching = false) }
