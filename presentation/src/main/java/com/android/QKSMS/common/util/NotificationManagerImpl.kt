...
class NotificationManagerImpl @Inject constructor(
...
) : com.android.QKSMS.manager.NotificationManager {
    ...
    override fun update(threadId: Long) {
        ...
        val contentIntent = Intent(context, ComposeActivity::class.java).putExtra("threadId", threadId).putExtra("locked", conversation.locked)
        ...
        messages.forEach { message ->
            ...
            var title = when (conversation.locked) {
                true -> "Locked"
                false -> message.getSummary()
            }
            NotificationCompat.MessagingStyle.Message(title, message.date, person.build()).apply {
