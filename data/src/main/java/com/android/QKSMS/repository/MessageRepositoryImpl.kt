 ...
 */
package com.android.QKSMS.repository
...
import com.android.QKSMS.common.util.extensions.now
import com.android.QKSMS.compat.TelephonyCompat
import com.android.QKSMS.extensions.anyOf
...
import com.android.QKSMS.util.ImageUtils
...
//            false -> signedBody
//        }
        val strippedBody = StripAccents.stripAccents(signedBody)

        val parts = smsManager.divideMessage(StripAccents.stripAccents(signedBody)).orEmpty()
        ...
        if (addresses.size == 1 && attachments.isEmpty() && !forceMms) { // SMS
            ...
            var name = ""
            parts += attachments
                    .mapNotNull { attachment -> attachment as? Attachment.Audio }
                    .map { attachment ->
                    ...
                    .map { audio ->
                        remainingBytes -= audio.size
                        MMSPart(name, ContentType.AUDIO_MP3, audio)
                    }
    ...
    override fun sendSms(message: Message) {
        ...
        val parts = smsManager
                .divideMessage(message.body)//if (prefs.unicode.get()) StripAccents.stripAccents(message.body) else message.body)
                ?: arrayListOf()
    ...
    override fun markAllUnlocked() {
        val realm = Realm.getDefaultInstance()
        val conversation = realm.where(Conversation::class.java).equalTo("locked", true).findAll()
        realm.executeTransaction { conversation.forEach { c -> c.locked = false } }
        realm.close()
    }
}